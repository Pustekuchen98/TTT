...
function ENT:Activate1( )
	...
	self.Entity:GetPhysicsObject():ApplyForceCenter( Vector(0,0,1) * 1750 );
	...
	timer.Create( "jump_timer"..tostring(self.Entity), 0.5, 1, self.MakeRope2, self )
...
function ENT:Think( )
	if self.Used then
		...
		if self.TryActivating then
			local vectmp = self.Entity:GetPhysicsObject( ):GetVelocity( )
			if vectmp.z > -100 then
				timer.Create( "activate_timer2"..tostring(self.Entity), 1, 1, self.Activate1, self )
...
function ENT:Initialize()
	...
	timer.Create( "activate_timer"..tostring(self.Entity), 1.75, 1, self.TryActivate, self )
